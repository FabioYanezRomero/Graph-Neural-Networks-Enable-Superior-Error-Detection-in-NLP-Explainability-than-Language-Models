...
for ds in "${DATASETS[@]}"; do
  ...
  if [[ -z "$MODEL_FOR_DATASET" && -n "$CHECKPOINT_PATH" ]]; then
    CONFIG_MODEL=$(SYN_COMBINED_WEIGHTS="$CHECKPOINT_PATH" python3 - <<'PY'
import json
import os
import sys
weights_path = os.environ.get('SYN_COMBINED_WEIGHTS')
if not weights_path:
    sys.exit(0)

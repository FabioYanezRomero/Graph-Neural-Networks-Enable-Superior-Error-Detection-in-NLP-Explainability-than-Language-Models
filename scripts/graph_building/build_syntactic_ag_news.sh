...
if [[ -n "$CHECKPOINT_PATH" && ! -f "$CHECKPOINT_PATH" ]]; then
  ...
  echo "[warn] No AG News checkpoint configured; using base model weights." >&2
fi

MODEL_FOR_DATASET="$GLOBAL_MODEL_PREF"
if [[ -z "$MODEL_FOR_DATASET" ]]; then
  if [[ -n "$CHECKPOINT_PATH" ]]; then
    CONFIG_MODEL=$(SYN_AG_WEIGHTS="$CHECKPOINT_PATH" python3 - <<'PY'
import json
import os
import sys
weights_path = os.environ.get('SYN_AG_WEIGHTS')
if not weights_path:
    sys.exit(0)

...
PLOT_HEIGHT = 480
FONT_SIZE = 10
COLOR_INSERTION = "#636EFA"
COLOR_DELETION = "#EF553B"
COLOR_GAP = "#00CC96"
COLOR_AAC = "#AB63FA"
...
METRIC_CONFIGS = {
    "normalised_insertion_auc": {"title": "Normalised Insertion AUC", "color": COLOR_INSERTION},
    "normalised_deletion_auc": {"title": "Normalised Deletion AUC", "color": COLOR_DELETION},
    "faithfulness_gap": {"title": "Faithfulness Gap", "color": COLOR_GAP},
    "deletion_aac": {"title": "Deletion AAC", "color": COLOR_AAC},
}
...
def prepare_metrics(df: pd.DataFrame) -> pd.DataFrame:
    if "normalised_deletion_auc" in df.columns:
        df["normalised_deletion_auc"] = parse_numeric_series(df["normalised_deletion_auc"])
    else:
        df["normalised_deletion_auc"] = np.nan

    if "normalised_insertion_auc" in df.columns:
        df["normalised_insertion_auc"] = parse_numeric_series(df["normalised_insertion_auc"])
    else:
        df["normalised_insertion_auc"] = np.nan

    if "deletion_aac" in df.columns:
        df["deletion_aac"] = parse_numeric_series(df["deletion_aac"])
    else:
        df["deletion_aac"] = np.nan

    df["faithfulness_gap"] = df["normalised_deletion_auc"] - df["normalised_insertion_auc"]
    return df
...
def process_csv(csv_path: Path, grid_points: int) -> None:
    ...
    if df.empty:
        print(f"[skip] Empty AUC CSV skipped: {csv_path}")
        return
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate KDEs and divergence metrics for AUC analytics.")
    parser.add_argument(
        "--base-dir",
        type=Path,
        default=BASE_INPUT_ROOT,
        help="Directory containing per-dataset AUC analytics CSV files.",
    )
    parser.add_argument(
        "--grid-points",
        type=int,
    ...
    csv_paths = discover_auc_csvs(base_dir)
    if not csv_paths:
        print("No AUC CSV files found.")
        return

    for csv_path in tqdm(csv_paths, desc="Generating AUC KDEs"):
        try:

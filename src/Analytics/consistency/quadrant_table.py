#!/usr/bin/env python3
"""Export dataset-level Consistency quadrant counts for tabular reporting."""
...
from Analytics.consistency.final_plot import (  # noqa: E402
    DATASET_LABELS,
    GRAPH_LABELS,
    METHOD_LABELS,
    PLOTS_ROOT,
...
def build_quadrant_table(dataset: str) -> Path:
    instances = load_instances(dataset)
    if instances.empty:
        raise ValueError(f"No consistency instances found for dataset '{dataset}'.")

    working = instances.copy()
    working["quadrant"] = working["quadrant"].fillna("Unknown")
    ...
    for method, graph in experiment_sequence:
        ...
        for segment_name, correctness_flag in SEGMENTS:
            ...
            for quadrant in QUADRANT_ORDER:
                count = int((segment_subset["quadrant"] == quadrant).sum())
    ...
    output_path = PLOTS_ROOT / dataset / f"consistency_quadrant_table_{dataset}.csv"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    summary.to_csv(output_path, index=False)
    print(f"✓ {dataset}: consistency quadrant table -> {output_path}")
    return output_path


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export Consistency quadrant counts and percentages per dataset.")
    parser.add_argument(
        "--dataset",
        action="append",
